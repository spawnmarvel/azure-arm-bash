...
echo "Please provide a user name and output dir"
echo "No user name, then it is whoami"
echo "No output dir, then it is /tmp"
...
if [ -z $2 ]; then
    ...
    mkdir /tmp/$2
    output=/tmp/$2/${user}_test_home_$(date +%Y-%m-%d_%H%M%S).tar.gz
fi
